...
class PedogeeShell {
  ...
  async initialize() {
    try {
      this.updateStatus("Loading DuckDB library...", "loading");
      ...
      const JSDELIVR_BUNDLES = this.duckdb.getJsDelivrBundles();

      // Select appropriate bundle for browser
      const bundle = await this.duckdb.selectBundle(JSDELIVR_BUNDLES);
  ...
  setupUI() {
    ...
    document
      ...
      .addEventListener("click", () => this.formatSQL());

    // Copy SQL button
    document
      .getElementById("btn-copy")
      .addEventListener("click", () => this.copySQL());
...
ORDER BY tree_count DESC;`;
  ...
  async exportResults(format) {
    ...
    try {
      ...
      if (format === "csv") {
        // Convert to CSV
        const rows = this.currentResults
          .toArray()
  ...
  formatSQL() {
    ...
    sql = sql.replace(
      /\b(SELECT|FROM|WHERE|GROUP BY|HAVING|ORDER BY|LIMIT)\b/gi,
      "\n$1",
    );
